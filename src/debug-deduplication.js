...
// Normalize sender name for consistent comparison between JSON and Native formats
  ...
  function buildSenderMapping(jsonMessages, nativeMessages) {
    ...
    const youStats = Array.from(senderStats.entries()).find(([sender, stats]) => stats.isYou);
    const phoneStats = Array.from(senderStats.entries()).filter(([sender, stats]) => stats.isPhone);
    const nameStats = Array.from(senderStats.entries()).filter(([sender, stats]) => !stats.isPhone && !stats.isYou);
    ...
    if (youStats) {
      ...
      for (const [sender, stats] of nameStats) {
        if (stats.count > 3) {
          const similarity = calculateMessageSimilarity(youMessages, stats.sampleMessages);
          if (similarity > 0.2) {
  ...
  function extractShortName(fullName) {
    ...
    name = name.replace(/\b(mock|interview|eng|frontend|backend|chat|with)\b/gi, "");
...
function debugDeduplication(chatDir) {
  ...
  const nativeMessages = parseNativeFile(nativePath);
    ...
    Array.from(senderMapping.entries()).slice(0, 5).forEach(([original, normalized]) => {
...
if (require.main === module) {
  ...
  const chatDir =
    args[0] ||
    "data/input/2025/1234567890___Test-Chat-Directory";
